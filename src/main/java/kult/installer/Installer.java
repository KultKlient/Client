package kult.installer;

import com.formdev.flatlaf.FlatDarkLaf;
import com.formdev.flatlaf.FlatLightLaf;
import kult.installer.layouts.VerticalLayout;
...
public class Installer {
    public static String API_URL = "https://kultklient.github.io/kultklient.github.io-api/";
    ...
    public void start() {
        ...
        if (newerFound) {
            System.out.println("There is a new version of KultKlient Installer, v" + Version.getLatest() + "! You are using v" + INSTALLER_VERSION + "!");
            JOptionPane.showMessageDialog(null, "There is a new version of KultKlient Installer, v" + Version.getLatest() + "! You are using v" + INSTALLER_VERSION + "!", "Newer KultKlient Installer found!", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            System.out.println("Failed to fetch installer metadata from the server!");
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Installer metadata parsing failed, please contact the KultKlient support team via Discord! \nError: " + e, "Metadata parsing failed!", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        JFrame frame = new JFrame("KultKlient Installer - v" + INSTALLER_VERSION);
        ...
        frame.setIconImage(new ImageIcon(Objects.requireNonNull(Utils.class.getClassLoader().getResource("assets/kultklient/textures/icons/icon.png"))).getImage());
        ...
        installHelpButton.addActionListener(event -> openUrl("https://kultklient.github.io/Installation/"));
        ...
        button.addActionListener(action -> {
            ...
            String jarName = "KultKlient-v" + selectedClientVersion + "-Fabric_" + selectedGameVersion + ".jar";
            String downloadURL = "https://kultklient.github.io/kultklient.github.io-api/Download/" + selectedGameVersion.replace(".", "-") + "/" + jarName;
                ...
                if ("progress".equals(event.getPropertyName())) progressBar.setValue((Integer) event.getNewValue());
                    ...
                    if (modsFolderContents != null) {
                        ...
                        if (modsFolder.exists() && modsFolder.isDirectory() && !isEmpty) {
                            boolean failedToRemoveKultKlient = false;
                            boolean shownKultKlientDialog = false;

                            for (File mod : modsFolderContents) {
                                String modName = mod.getName().toLowerCase();
                                if (!shownKultKlientDialog && !modName.contains("installer") && (modName.contains("kultklient"))) {
                                    int result = JOptionPane.showOptionDialog(frame, "Another installation of KultKlient was found in your mods folder. Do you want to remove it, or cancel the installation? \n\nFile Name: " + mod.getName(), "Installed KultKlient Detected", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, new String[] {"Yes", "Cancel"}, "Yes");

                                    shownKultKlientDialog = true;
                                    if (result != JOptionPane.YES_OPTION) {
                                        cancelled = true;
                                        break;
                                    }

                                    if (!mod.delete()) failedToRemoveKultKlient = true;
                                }
                            }

                            if (failedToRemoveKultKlient) {
                                System.out.println("Failed to remove KultKlient from mods folder to update them!");
                                JOptionPane.showMessageDialog(frame, "Failed to remove KultKlient from your mods folder to update them, please make sure your game is closed and try again!", "Failed to prepare KultKlient for update", JOptionPane.ERROR_MESSAGE);
                                cancelled = true;
                            }
                        }
                            ...
                            for (File mod : modsFolderContents) {
                                ...
                                if (modName.contains("optifine") || modName.contains("optifabric")) {
                                    if (!shownOptifineDialog) {
                                        int result = JOptionPane.showOptionDialog(frame,"Optifine was found in your mods folder, but Optifine is incompatible with KultKlient. Do you want to remove it, or cancel the installation? \n\nFile Name: " + mod.getName(), "Installed Optifine Detected", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, new String[] {"Yes", "Cancel"}, "Yes");
    ...
    public String getStorageDirectoryName() {
        String os = System.getProperty("os.name").toLowerCase();
        if (os.contains("mac")) return "kultklient-installer";
        else return ".kultklient-installer";
    }
