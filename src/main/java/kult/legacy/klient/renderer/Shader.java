...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            KultKlientLegacy.LOG.error(KultKlientLegacy.logPrefix + "Failed to compile vertex shader (" + vertPath + "): " + vertError);
            throw new RuntimeException("[KultKlient Legacy] Failed to compile vertex shader (" + vertPath + "): " + vertError);
        }
        ...
        if (fragError != null) {
            KultKlientLegacy.LOG.error(KultKlientLegacy.logPrefix + "Failed to compile fragment shader (" + fragPath + "): " + fragError);
            throw new RuntimeException("[KultKlient Legacy] Failed to compile fragment shader (" + fragPath + "): " + fragError);
        }
        ...
        if (programError != null) {
            KultKlientLegacy.LOG.error(KultKlientLegacy.logPrefix + "Failed to link program: " + programError);
            throw new RuntimeException("[KultKlient Legacy] Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(new Identifier("kultklientlegacy", "shaders/" + path)).getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
