...
public class AccountCache implements ISerializable<AccountCache> {
    ...
    public boolean loadHead(String url) {
        try {
            ...
            KultKlientLegacy.LOG.error(KultKlientLegacy.logPrefix + "Failed to read skin url (" + url + ").");
            return false;
        }
    }
    ...
    public static void loadSteveHead() {
        try {
            ByteBuffer data = TextureUtil.readResource(mc.getResourceManager().getResource(new Identifier("kultklientlegacy", "textures/steve.png")).getInputStream());
            data.rewind();
