...
import kultklient.legacy.client.mixin.CloseHandledScreenC2SPacketAccessor;
import kultklient.legacy.client.mixin.HandledScreenAccessor;
...
import kultklient.legacy.client.utils.player.InventorySorter;
...
public class InventoryTweaks extends Module {
    ...
    public void steal(ScreenHandler handler) {
        KultKlientExecutor.execute(() -> moveSlots(handler, 0, getRows(handler) * 9));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = getRows(handler) * 9;
        KultKlientExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9));
    }
