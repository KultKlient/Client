...
public class GuiThemes {
    private static final File FOLDER = new File(KultKlientLegacy.VERSION_FOLDER, "GUI");
    ...
    public static void postInit() {
        ...
        if (theme == null) select("KultKlient");
    }
    ...
    public static void add(GuiTheme theme) {
        for (Iterator<GuiTheme> it = themes.iterator(); it.hasNext();) {
            if (it.next().name.equals(theme.name)) {
                ...
                KultKlientLegacy.LOG.error(KultKlientLegacy.logPrefix + "Theme with the name '{}' has already been added.", theme.name);
                break;
            }
        }
