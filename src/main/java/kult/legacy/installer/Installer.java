...
public class Installer {
    ...
    public void start() {
        ...
        try {
            ...
            System.out.println("Failed to fetch installer metadata from the server!");
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Installer metadata parsing failed, please contact the KultKlient Legacy support team via Discord! \nError: " + e, "Metadata Parsing Failed!", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        JFrame frame = new JFrame("KultKlient Legacy Installer");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLayout(new BorderLayout());
        frame.setSize(350,350);
        frame.setLocationRelativeTo(null);
        frame.setIconImage(new ImageIcon(Objects.requireNonNull(Utils.class.getClassLoader().getResource("assets/kultklientlegacy/textures/icons/icon.png"))).getImage());
        ...
        button.addActionListener(action -> {
            ...
            String jarName = "KultKlient_Legacy-v" + selectedClientVersion + "-Fabric_" + selectedGameVersion + ".jar";
                ...
                if ("progress".equals(event.getPropertyName())) progressBar.setValue((Integer) event.getNewValue());
                    ...
                    if (modsFolderContents != null) {
                        ...
                        if (modsFolder.exists() && modsFolder.isDirectory() && !isEmpty) {
                            int result = JOptionPane.showConfirmDialog(frame,"An existing mods folder was found in the selected game directory. Do you want to update/install KultKlient Legacy?", "Mods Folder Detected", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                            ...
                            for (File mod : modsFolderContents) {
                                if (mod.getName().toLowerCase().contains("optifine") || mod.getName().toLowerCase().contains("optifabric")) {
                                    if (!shownOptifineDialog) {
                                        int result = JOptionPane.showOptionDialog(frame,"Optifine was found in your mods folder, but Optifine is incompatible with KultKlient Legacy. Do you want to remove it, or cancel the installation?", "Optifine Detected", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, new String[] {"Yes", "Cancel"}, "Yes");
    ...
    public String getStorageDirectoryName() {
        String os = System.getProperty("os.name").toLowerCase();
        if (os.contains("mac")) return "kultklient-legacy-installer";
        else return ".kultklient-legacy-installer";
    }
