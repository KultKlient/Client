...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            KultKlient.LOG.error(KultKlient.logPrefix + "Failed to compile vertex shader (" + vertPath + "): " + vertError);
            throw new RuntimeException("[KultKlient] Failed to compile vertex shader (" + vertPath + "): " + vertError);
        }
        ...
        if (fragError != null) {
            KultKlient.LOG.error(KultKlient.logPrefix + "Failed to compile fragment shader (" + fragPath + "): " + fragError);
            throw new RuntimeException("[KultKlient] Failed to compile fragment shader (" + fragPath + "): " + fragError);
        }
        ...
        if (programError != null) {
            KultKlient.LOG.error(KultKlient.logPrefix + "Failed to link program: " + programError);
            throw new RuntimeException("[KultKlient] Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(new Identifier("kultklient", "shaders/" + path)).getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
