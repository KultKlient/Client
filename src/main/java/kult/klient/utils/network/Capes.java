package kult.klient.utils.network;
...
public class Capes {
    ...
    public static void init() {
        ...
        KultKlientExecutor.execute(() -> {
            // Cape owners
            Stream<String> lines = HTTP.get(KultKlient.API_URL + "Cape/capeowners").sendLines();
            ...
            // Capes
            lines = HTTP.get(KultKlient.API_URL + "Cape/capes").sendLines();
            if (lines != null) lines.forEach(s -> {
                String[] split = s.split(" ");
        ...
        KultKlient.EVENT_BUS.subscribe(Capes.class);
    }
