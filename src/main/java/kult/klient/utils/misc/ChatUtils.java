...
import kult.klient.utils.render.color.RainbowColor;
import kult.klient.KultKlient;
import kult.klient.mixin.ChatHudAccessor;
...
public class ChatUtils {
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        ...
        message.append(getKultKlientPrefix());
    ...
    private static BaseText getKultKlientPrefix() {
        ClientSpoof cs = Modules.get().get(ClientSpoof.class);

        BaseText text = new LiteralText("KultKlient");
        ...
        else text.setStyle(text.getStyle().withColor(KultKlient.INSTANCE.KULTKLIENT_COLOR.getPacked()));
