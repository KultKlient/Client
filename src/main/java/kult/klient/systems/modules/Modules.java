package kult.klient.systems.modules;

import com.google.common.collect.Ordering;
import com.mojang.serialization.Lifecycle;
import kult.klient.KultKlient;
import kult.klient.events.kultklient.ActiveModulesChangedEvent;
...
import kult.klient.systems.modules.chat.*;
import kult.klient.systems.modules.client.*;
import kult.klient.systems.modules.combat.*;
import kult.klient.systems.modules.crash.*;
import kult.klient.systems.modules.misc.*;
import kult.klient.systems.modules.movement.*;
import kult.klient.systems.modules.player.*;
import kult.klient.systems.modules.render.*;
import kult.klient.systems.modules.world.*;
...
import kult.klient.systems.modules.client.swarm.Swarm;
import kult.klient.systems.modules.movement.elytrafly.ElytraFly;
import kult.klient.systems.modules.movement.speed.Speed;
import kult.klient.systems.modules.render.marker.Marker;
import kult.klient.systems.modules.render.search.Search;
import kult.klient.utils.Utils;
import kult.klient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                KultKlient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    void removeActive(Module module) {
        synchronized (active) {
            if (active.remove(module)) KultKlient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        if (moduleToBind != null && moduleToBind.keybind.canBindTo(isKey, value)) {
            ...
            KultKlient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
            moduleToBind = null;
            return true;
        }
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    KultKlient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    KultKlient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
    ...
    public static class ModuleRegistry extends Registry<Module> {
        public ModuleRegistry() {
            super(RegistryKey.ofRegistry(new Identifier("kultklient", "modules")), Lifecycle.stable());
        }
