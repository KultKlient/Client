...
public class SBlock {
    ...
    public void update() {
        state = KultKlient.mc.world.getBlockState(blockPos.set(x, y, z));
        neighbours = 0;
    ...
    private boolean isNeighbour(Direction dir) {
        ...
        BlockState neighbourState = KultKlient.mc.world.getBlockState(blockPos);

        if (neighbourState.getBlock() != state.getBlock()) return false;

        VoxelShape shape = state.getOutlineShape(KultKlient.mc.world, blockPos);
        VoxelShape neighbourShape = neighbourState.getOutlineShape(KultKlient.mc.world, blockPos);
    ...
    private boolean isNeighbourDiagonal(double x, double y, double z) {
        ...
        return state.getBlock() == KultKlient.mc.world.getBlockState(blockPos).getBlock();
    }
