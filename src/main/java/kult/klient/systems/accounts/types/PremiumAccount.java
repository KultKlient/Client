...
public class PremiumAccount extends Account<PremiumAccount> {
    ...
    public boolean login() {
        ...
        try {
            ...
            KultKlient.LOG.error(KultKlient.logPrefix + "Failed to contact the authentication server.");
            return false;
        } catch (AuthenticationException e) {
            if (e.getMessage().contains("Invalid username or password") || e.getMessage().contains("account migrated"))
                KultKlient.LOG.error(KultKlient.logPrefix + "Wrong password.");
            else KultKlient.LOG.error(KultKlient.logPrefix + "Failed to contact the authentication server.");
            return false;
        }
    }

    public YggdrasilUserAuthentication getAuth() {
        YggdrasilUserAuthentication auth = (YggdrasilUserAuthentication) new YggdrasilAuthenticationService(((MinecraftClientAccessor) KultKlient.mc).getProxy(), "").createUserAuthentication(Agent.MINECRAFT);
