...
import kult.klient.systems.accounts.AccountUtils;
...
public class TheAlteningAccount extends Account<TheAlteningAccount> {
    ...
    public boolean login() {
        YggdrasilMinecraftSessionService service = (YggdrasilMinecraftSessionService) KultKlient.mc.getSessionService();
        ...
        try {
            ...
            KultKlient.LOG.error(KultKlient.logPrefix + "Failed to login with The Altening.");
            return false;
        }
    }

    private YggdrasilUserAuthentication getAuth() {
        YggdrasilUserAuthentication auth = (YggdrasilUserAuthentication) new YggdrasilAuthenticationService(((MinecraftClientAccessor) KultKlient.mc).getProxy(), "", Environment.create(AUTH, ACCOUNT, SESSION, SERVICES, "The Altening")).createUserAuthentication(Agent.MINECRAFT);

        auth.setUsername(name);
        auth.setPassword("KultKlient on top!");
