...
public class WatermarkHud extends TripleTextHudElement {
    ...
    private static final Identifier KULTKLIENT_LOGO = new Identifier("kultklient", "textures/icons/icon.png");
    private static final Identifier METEOR_LOGO = new Identifier("kultklient", "textures/icons/meteor.png");
    ...
    private final Setting<Boolean> updateCheck = sgGeneral.add(new BoolSetting.Builder()
        .name("update-checker")
        .description("Checks if a new version of KultKlient is available.")
        .defaultValue(true)
        .build()
    );

    public WatermarkHud(HUD hud) {
        super(hud, "watermark", "Displays a KultKlient watermark.", true);
    }
