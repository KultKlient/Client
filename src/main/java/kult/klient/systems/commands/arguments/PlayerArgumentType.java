...
public class PlayerArgumentType implements ArgumentType<PlayerEntity> {
    ...
    static {
        if (KultKlient.mc.world != null) {
            EXAMPLES = KultKlient.mc.world.getPlayers()
                .stream()
                .limit(3)
    ...
    public PlayerEntity parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (PlayerEntity p : KultKlient.mc.world.getPlayers()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(KultKlient.mc.world.getPlayers().stream().map(PlayerEntity::getEntityName), builder);
    }
